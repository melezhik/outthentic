...
B<Create a story file>. Story is just an any perl script that yields something into stdout:
...
B<Create a story check file>. Story check is a bunch of lines stdout should match. Here we require to have `I am OK' and `I am outthentic' lines in stdout:
...
B<Run a story>. Story runner is script that parses and then executes stories, it:
...
B<Compilation phase> where stories are converted into perl test files.


=item -

B<Execution phase> where perl test files are recursively executed by prove.


=back


=head2 Compilation phase

When story runner iterate through story files and story check it convert both of them into single perl test file.
Perl test file I<looks like> list of Test::More asserts.
...
It's convenient to talk about a I<check list> - is list of none empty lines in a story check file.

Basically every item of check list is so called  I<check expression>.
...
Story runner does not care about I<how many times> a given check expression is found in stdout.
...
=head2 Comments, blank lines and text blocks
...
Perl expressions are just a pieces of perl code to I<get evaled> inside your story. This is how it works:
...
Generators is the way to I<populate story check lists on the fly>.
...
Story runner adds `projectI<root>directory/lib' path to PERL5LIB path, so you may define some modules here and then `use' them inside your story check:
...
`captures()' function is used to access captured data array, it returns an array reference holding all chunks captured during I<latest regular expression check>.
